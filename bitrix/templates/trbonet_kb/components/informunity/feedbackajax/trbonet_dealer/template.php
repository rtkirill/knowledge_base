<? if (!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED !== true) die(); ?><div id="form_dealer">
	<? if (strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_REQUEST['AJAX_FORM'] == 'dealer') $APPLICATION->RestartBuffer(); ?>
	<div class="tn_form__wrapper l-constrained l-flow">
		<? 
		$auto_focus = 0;
		$FL_req = 0;
		?>
		<form method="POST" id="connect" class="tn_form l-form" enctype="multipart/form-data">
			<?= bitrix_sessid_post() ?>
			<? $cp = 0; ?>
			...
			<? else: ?>
				...
				<div class="l-form__left">
					...
					<? foreach ($arParams["NEW_EXT_FIELDS"] as $i => $field): ?>
						...
						<? if ($i == 'COUNTRY' || $i == 'INTEREST' || $i == 'INDUSTRY'): ?>
				...
				<div class="l-form__bottom">
					<div class="tn_form__item tn_form__submit l-center">
						<input id="dealer_submit" type="submit" value="Send Request" name="submit" class="tn_button tn_button--thik">
					</div>
				</div>
			<? endif ?>
	...
	<? if (strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && $_REQUEST['AJAX_FORM'] == 'dealer') {
			CEvent::CheckEvents();
			die();
	} ?>
</div>
<script>
$(function() {
	var dealerForm = new af_dealer({
		root: '#form_dealer',
		form: '#form_dealer form',
